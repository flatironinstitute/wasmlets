...
CFLAGS+=-Og --profiling
LDFLAGS+=-sASSERTIONS=0 -sCHECK_NULL_WRITES=0 -sDEFAULT_TO_CXX=0 

src/wasm/wavelib.js: #wavelib.o wavelib/build_wasm/Bin/RelWithDebInfo/libwavelib.a
	$(LINK.c) -o $@ wavelib.c wavelib/src/*.c --emit-tsd intf.d.ts

wavelib.o: wavelib.c
	$(COMPILE.c) -o $@ $<

wavelib/build_wasm/Bin/RelWithDebInfo/libwavelib.a:
	mkdir -p wavelib/build_wasm
	cd wavelib/build_wasm && emcmake cmake .. -DCMAKE_BUILD_TYPE=RelWithDebInfo -DCMAKE_CXX_COMPILER=$(CXX) -DCMAKE_C_COMPILER=$(CC) -DCMAKE_C_FLAGS="$(CFLAGS)" -DCMAKE_CXX_FLAGS="$(CXXFLAGS)"
