...
src/wasm/wavelib.js: wavelib.o wavelib/build_wasm/Bin/Release/libwavelib.a
	$(LINK.c) -o $@ $^ --emit-tsd intf.d.ts

wavelib.o: wavelib.c
	$(COMPILE.c) -o $@ $<

wavelib/build_wasm/Bin/Release/libwavelib.a:
	mkdir -p wavelib/build_wasm
	cd wavelib/build_wasm && emcmake cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_COMPILER=$(CXX) -DCMAKE_C_COMPILER=$(CC) -DCMAKE_CXX_FLAGS="$(CXXFLAGS)"
